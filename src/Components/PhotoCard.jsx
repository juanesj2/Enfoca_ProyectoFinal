...
export default function PhotoCard({ foto }) {
    ...
    const toggleLike = async () => {
        ...
        try {
            if (liked) {
                // Quitar like
                const response = await axios.delete(`/fotografias/${foto.id}/like`);
                setLiked(false);
        ...
        <div className="glass-panel overflow-hidden group hover-lift flex flex-col h-full hover:shadow-2xl transition-all duration-300">
            ...
            <div className="relative aspect-[4/3] overflow-hidden bg-gray-200 dark:bg-gray-800">
                ...
                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                    <Link to={`/comentar?fotografia_id=${foto.id}`} className="bg-white/20 backdrop-blur-md border border-white/50 text-white px-6 py-2 rounded-full font-medium shadow-lg hover:bg-white/30 transition-colors">
                        Ver Comentarios
                    </Link>
                </div>
            </div>
            ...
            <div className="p-5 flex flex-col flex-grow">
                ...
                <div className="flex items-center justify-between mt-auto pt-4 border-t border-gray-100 dark:border-gray-800">
                    ...
                    <div className="flex gap-3 text-gray-400">
                        ...
                        <Link to={`/comentar?fotografia_id=${foto.id}`} className="flex items-center gap-1 hover:text-indigo-500 transition-colors cursor-pointer">
