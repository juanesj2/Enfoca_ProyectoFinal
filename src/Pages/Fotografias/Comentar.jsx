...
export default function Comentar() {
    ...
    const fotografiaId = searchParams.get('fotografia_id');
    ...
    const toggleLike = async () => {
        if (isLiking || !fotografia) return;
        setIsLiking(true);
        try {
            if (liked) {
                const response = await axios.delete(`/fotografias/${fotografia.id}/like`);
                setLiked(false);
                setLikesCount(response.data.likesCount || likesCount - 1);
            } else {
            ...
            alert('Error al dar me gusta a esta foto. Inicia sesión.');
        } finally {
            setIsLiking(false);
        }
    };
    ...
    const submitComentario = async (e) => {
        e.preventDefault();
        if (!nuevoComentario.trim() || !fotografia) return;
        ...
        try {
            ...
            const resComentarios = await axios.get(`/fotografias/${fotografia.id}/comentarios`);
            setComentarios(resComentarios.data.data || resComentarios.data);
        } catch (error) {
    ...
    return (
        <MainLayout>
            <div className="flex justify-between items-center mb-6">
                <Link to="/fotografias" className="flex items-center text-gray-500 hover:text-indigo-600 dark:text-gray-400 font-medium transition-colors">
                ...
                <div className="lg:col-span-7 xl:col-span-8 flex flex-col">
                    <div className="glass-panel overflow-hidden flex flex-col group">
                        ...
                        <div className="p-4 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between bg-white/50 dark:bg-gray-900/50">
                            ...
                            {/* Opciones (Reportar) */}
                            <Link to={`/reportes/create/${fotografia.id}`} className="text-gray-400 hover:text-red-500 transition-colors p-2 rounded-full hover:bg-red-50 dark:hover:bg-red-900/20" title="Reportar Imagen">
                        ...
                        <div className="p-4 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center bg-white/50 dark:bg-gray-900/50">
                            <div className="flex gap-4">
                                ...
                                <div className="flex items-center gap-1.5 text-gray-500 dark:text-gray-400">
                                    ...
                                    <span className="font-semibold text-lg">{comentarios.length}</span>
                                </div>
                            </div>
