...
export default function MisFotografias() {
    const defaultImageFallback = '/images/placeholder.png';
    const [fotos, setFotos] = useState([]);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        document.title = "Mis Fotografías - Enfoca";
        fetchMisFotografias();
    }, []);

    const fetchMisFotografias = async () => {
        setLoading(true);
        try {
            const res = await axios.get('/mis-fotos');
            // Dependiendo de si la API devuelve paginación o un array directo
            const data = res.data.data || res.data;
            setFotos(Array.isArray(data) ? data : Object.values(data));
        } catch (error) {
            console.error("Error fetching mis fotografias", error);
        } finally {
            setLoading(false);
        }
    };

    const deleteFotografia = async (id, e) => {
        e.preventDefault();
        if (window.confirm('¿Estás seguro de que deseas eliminar tu publicación? Esto no se puede deshacer.')) {
            try {
    ...
    const downloadPDF = (id, e) => {
        ...
        window.open(`${baseUrl}/fotografia/${id}/pdf`, '_blank');
    };

    const sendEmail = async (id, e) => {
        e.preventDefault();
        try {
            await axios.post('/enviar-correo', { foto_id: id });
            alert("¡Correo enviado exitosamente con los detalles!");
        } catch (error) {
            console.error("Error sending email", error);
            alert("Hubo un error al intentar enviar el correo. Verifica tu configuración.");
        }
    };
        ...
        <MainLayout>
            <div className="max-w-7xl mx-auto py-8">
                ...
                <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
                    ...
                    <Link to="/fotografias/create" className="bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 text-white font-medium px-6 py-2.5 rounded-full shadow-lg hover:shadow-xl transition-all flex items-center gap-2 hover-lift whitespace-nowrap">
                    ...
                    <>
                        ...
                        {fotos.length > 0 ? (
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 animate-fade-in-up">
                                {fotos.map((foto) => {
                                    ...
                                    <div key={foto.id} className="glass-panel group overflow-hidden border border-gray-200 dark:border-gray-800 transition-all hover:shadow-2xl hover-lift flex flex-col">
                                        {/* Contenedor Imagen */}
                                        <Link to={`/comentar?fotografia_id=${foto.id}`} className="relative aspect-square block overflow-hidden bg-gray-100 dark:bg-gray-800">
                                            <img 
                                                src={imageUrl} 
                            ...
                            <div className="bg-white dark:bg-gray-900 rounded-3xl p-12 text-center border border-gray-100 dark:border-gray-800 shadow-sm mt-12 animate-slide-up">
                                ...
                                <Link to="/fotografias/create" className="inline-flex bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 text-white font-medium px-8 py-3 rounded-full shadow-lg hover:shadow-xl transition-all items-center gap-2 hover-lift">
