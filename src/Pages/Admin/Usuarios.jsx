...
export default function Usuarios() {
    ...
    useEffect(() => {
        document.title = "Control de Usuarios - Enfoca";
        fetchUsuarios();
    }, []);
    ...
    const handleDelete = async (id, e) => {
        e.preventDefault();
        if (window.confirm("¿Estás seguro de que quieres eliminar a este usuario? Esta acción es irreversible y eliminará todos sus datos asociados.")) {
            try {
            ...
            <div className="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-10 animate-fade-in-up">
                    <div className="flex items-center gap-4">
                        <Link to="/admin" className="p-2.5 bg-white dark:bg-gray-800 text-gray-500 hover:text-indigo-600 dark:text-gray-400 dark:hover:text-indigo-400 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 transition-all hover:-translate-x-1">
                        ...
                        <div>
                            <h1 className="text-3xl font-extrabold text-gray-900 dark:text-white capitalize">Control de Usuarios</h1>
                            <p className="text-gray-500 dark:text-gray-400 mt-1">Gestiona los miembros de la plataforma</p>
                        </div>
                    </div>
                </div>

                {/* Tabla de Usuarios */}
                <div className="glass-panel overflow-hidden border border-gray-200 dark:border-gray-800 rounded-3xl shadow-xl animate-fade-in-up" style={{ animationDelay: '100ms' }}>
                    {isLoading ? (
                        ...
                        <div className="overflow-x-auto">
                            <table className="min-w-full w-full">
                                <thead className="bg-gray-50/80 dark:bg-gray-800/80 backdrop-blur-sm border-b border-gray-200 dark:border-gray-800">
                                    <tr>
                                        <th className="px-6 py-4 font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider text-xs text-left">Usuario</th>
                                        <th className="px-6 py-4 font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider text-xs text-left">Email</th>
