...
export default function MisDesafios() {
    const [misDesafios, setMisDesafios] = useState({ data: [], meta: {} });
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        document.title = "Mis Logros - Enfoca";
        fetchMisDesafios();
    }, []);

    const fetchMisDesafios = async (url = '/desafios/mis-desafios') => {
        setLoading(true);
        try {
            const requestUrl = url.startsWith('http') ? url.replace(/.*\/api/, '') : url;
            const res = await axios.get(requestUrl);
            setMisDesafios(res.data);
            window.scrollTo({ top: 0, behavior: 'smooth' });
        } catch (error) {
            console.error("Error fetching mis desafios", error);
        } finally {
            setLoading(false);
        }
    };
                ...
                <div className="flex flex-col md:flex-row justify-between items-end mb-8 border-b border-gray-200 dark:border-gray-800 pb-6 gap-4">
                    <div>
                        ...
                        <p className="text-gray-500 dark:text-gray-400">
                            Has coleccionado {misDesafios.meta?.total || misDesafios.data?.length || 0} de los retos disponibles de la plataforma.
                        </p>
                    </div>
                    <Link to="/desafios" className="text-sm font-medium text-indigo-600 hover:text-indigo-700 dark:text-indigo-400 dark:hover:text-indigo-300 bg-indigo-50 dark:bg-indigo-900/20 px-5 py-2.5 rounded-full transition-colors flex items-center gap-2">
                    ...
                    <>
                        ...
                        {!misDesafios.data || misDesafios.data.length === 0 ? (
                            <div className="bg-white dark:bg-gray-900 rounded-3xl p-12 text-center border border-gray-100 dark:border-gray-800 shadow-sm animate-fade-in">
                                ...
                                <Link to="/fotografias" className="inline-flex bg-indigo-600 hover:bg-indigo-700 text-white font-medium px-6 py-2.5 rounded-full shadow-md transition-colors">
                            ...
                            <div className="space-y-4">
                                        ...
                                        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-5 text-center sm:text-left">
                                            ...
                                            <div className="py-1">
                                                ...
                                                <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-green-50 text-green-700 dark:bg-green-900/30 dark:text-green-400 border border-green-200 dark:border-green-800">
                                                    ...
                                                    {/* En Laravel Resource, el pivot podría no estar disponible, si está lo mostramos */}
                                                    {desafio.pivot?.conseguido_en ? `Conseguido el ${new Date(desafio.pivot.conseguido_en).toLocaleDateString()}` : '¡Conseguido!'}
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                {misDesafios.meta && misDesafios.meta.links && misDesafios.meta.links.length > 3 && (
                                    <div className="mt-8 flex justify-center gap-2">
                                        {misDesafios.meta.links.map((link, i) => (
                                            <button 
                                                key={i}
                                                onClick={() => fetchMisDesafios(link.url)}
                                                disabled={!link.url}
